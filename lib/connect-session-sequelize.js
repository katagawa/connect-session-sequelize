...
const SerializerDefault = require('./serializer/default')
const SerializerEncrypting = require('./serializer/encrypting')
...
module.exports = function SequelizeSessionInit (Store) {
  class SequelizeStore extends Store {
    constructor (options) {
      ...
      if (options.secret) {
        debug('Using encrypting serializer.')
        this.serializer = new SerializerEncrypting(options.secret)
      } else {
        debug('Using default serializer.')
    ...
    set (sid, data, fn) {
      debug('INSERT "%s"', sid)
      const serializer = this.serializer
      const stringData = serializer.serialize(data)
      ...
      return this.sessionModel
        ...
        .spread(function sessionCreated (session) {
          ...
          if (session.data !== stringData) {
            session.data = serializer.serialize(data)
            changed = true
          }
          if (changed) {
